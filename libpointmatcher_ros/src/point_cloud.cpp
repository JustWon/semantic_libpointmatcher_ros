...
namespace PointMatcher_ros
{
	...
	typename PointMatcher<T>::DataPoints rosMsgToSemanticPointMatcherCloud(const sensor_msgs::PointCloud2& rosMsg)
	{
		...
		for(auto it(rosMsg.fields.begin()); it != rosMsg.fields.end(); ++it, ++fieldId)
		{
			...
			// semantic property fill
			if (it->name == "intensity") {
				// get view for editing data
				View view(cloud.getDescriptorViewByName("semantic"));
				// use view to read data
				int ptId(0);
				for (size_t y(0); y < rosMsg.height; ++y)
				{
					...
					{
						const uint8_t* fPtr(dataPtr + it->offset);
						view(0, ptId) = T(*reinterpret_cast<const float*>(fPtr));
						dataPtr += rosMsg.point_step;
						ptId += 1;
					}
				}
	...
	template
	PointMatcher<float>::DataPoints rosMsgToSemanticPointMatcherCloud<float>(const sensor_msgs::PointCloud2& rosMsg);
	template
